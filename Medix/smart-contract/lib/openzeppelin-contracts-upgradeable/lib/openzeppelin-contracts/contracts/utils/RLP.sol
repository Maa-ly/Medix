    ...
    function _encode(bytes memory input, uint256 offset) private pure returns (bytes memory result) {
        ...
        if (length <= SHORT_THRESHOLD) {
            ...
            assembly ("memory-safe") {
                ...
                {
                    let dest := add(result, 0x21)
                    let src := add(input, 0x20)
                    let len := length
                    let full := and(len, not(0x1f))
                    for {
                        let i := 0
                    } lt(i, full) {
                        i := add(i, 0x20)
                    ...
                    if rem {
                        let srcpart := mload(add(src, full))
                        let destpart := mload(add(dest, full))
                        let mask := sub(shl(mul(8, sub(0x20, rem)), 1), 1)
                        mstore(add(dest, full), or(and(destpart, mask), and(srcpart, not(mask))))
                    }
                } // input
                mstore(0x40, add(result, add(length, 0x21))) // reserve memory
            }
            ...
            assembly ("memory-safe") {
                ...
                {
                    let dest := add(result, add(lenlength, 0x21))
                    let src := add(input, 0x20)
                    let len := length
                    let full := and(len, not(0x1f))
                    for {
