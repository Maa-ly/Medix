...
export function MintNFTModal({
  ...
  const { address, addToast, addCompletion } = useAppStore();
  ...
  const handleMint = async () => {
    ...
    if (!address) {
      addToast({
        type: "error",
        message: "Minting requires a wallet address. Please connect Wallet.",
      });
      return;
    }
    ...
    try {
      ...
      const isInsufficientFunds =
        ...
        shortMessage.toLowerCase().includes("insufficient funds") ||
        causeMessage.toLowerCase().includes("insufficient funds") ||
        stringifiedError.includes("insufficient funds");

      if (isInsufficientFunds) {
        addToast({
          type: "error",
          message: "Insufficient funds! You need Lisk Sepolia testnet ETH. Get some from the faucet: https://sepolia-faucet.lisk.com",
          duration: 8000,
        });
