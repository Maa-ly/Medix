...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    const detectCurrentSite = async () => {
      try {
        ...
        console.log("[Medix Popup] Current site detection:", siteInfo);
        setCurrentSite(siteInfo);
      } catch (error) {
        console.error("[Medix] Error detecting current site:", error);
      }
    };
      ...
      try {
        chrome.storage?.local?.get(["activeTracking"], (result) => {
          if (chrome.runtime.lastError) {
            console.error("[Medix] Storage error:", chrome.runtime.lastError);
            return;
          }
          console.log("[Medix Popup] Loaded tracking data:", {
          ...
          if (result?.activeTracking && Array.isArray(result.activeTracking)) {
            result.activeTracking.forEach((track: TrackedMedia) => {
              console.log("[Medix Popup] Updating tracking item:", track);
              useAppStore.getState().updateActiveTracking(track);
            });
          }
        });
      } catch (err) {
        console.error("[Medix] Failed to load active tracking:", err);
      }
    };
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (response?.success && response.data) {
            console.log("[Medix] Active sessions:", response.data);
          }
        }
      );
    } catch (err) {
      console.error("[Medix] Failed to get active sessions:", err);
    }
  ...
  const handleEnableSite = async () => {
    ...
    try {
      ...
      console.error("[Medix] Failed to enable tracking:", error);
      addToast({
        type: "error",
  ...
  console.log("[Medix Popup] All tracked media:", {
  ...
  return (
    <div className="px-4 py-6 space-y-8">
      {/* Test Data Button - Development/Testing */}
      {isConnected && (
        <div className="flex justify-end">
          <button
            onClick={() => setShowTestList(!showTestList)}
            className="px-3 py-1.5 bg-violet/20 hover:bg-violet/30 text-violet text-sm rounded-lg transition-colors flex items-center gap-2"
          >
        ...
        {showTestList && (
          ...
          >
            <motion.div
              ...
              className="bg-dark-900 rounded-2xl border border-white/10 p-6 max-w-md w-full"
            ...
            >
              ...
              <p className="text-sm text-gray-400 mb-4">
                Select test media to add to pending mints for testing the NFT minting flow
              </p>
              <div className="space-y-2">
                {testDataList.map((test, index) => (
                  <button
                    ...
                    className="w-full p-3 bg-dark-800 hover:bg-dark-700 rounded-xl transition-colors text-left group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-white truncate group-hover:text-violet transition-colors">
                          {test.title}
                        </p>
                        <p className="text-xs text-gray-400 capitalize">{test.type} • {test.platform}</p>
                      </div>
                      <Plus className="w-5 h-5 text-gray-400 group-hover:text-violet transition-colors" />
                    </div>
                  </button>
                ))}
              </div>
