...
export function CommunityPage() {
  const { isConnected, addToast, address } = useAppStore();
  ...
  const toggleFriend = (userId: string, userProfile: UserProfile | null) => {
    // Copy wallet address or profile name
    const textToCopy = userProfile?.profileName || userId;
    ...
    async function loadMatches() {
      ...
      try {
        if (!isConnected || !address) {
          console.log("[Medix] Matches: Not connected or no address", {
            isConnected,
            address: address,
          });
        ...
        console.log("[Medix] Loading matches for:", address);

        const ids = await readUserNfts(address as `0x${string}`);
        console.log("[Medix] User NFT IDs:", ids.length, ids.map(String));
        setUserNftIds(ids);

        if (ids.length === 0) {
          console.log("[Medix] User has no NFTs, no matches possible");
          setMatchingAddrs([]);
          setIsLoadingMatches(false);
          return;
        }
  ...
  }, [activeTab, isConnected, address, addToast]);
            ...
            {isLoadingMatches ? (
              ...
              <div className="space-y-3">
                {matchingAddrs.map((addr, index) => {
                  ...
                  // Display name priority: profileName > truncated address
                  const displayName = userProfile?.profileName || `${addr.slice(0, 6)}...${addr.slice(-4)}`;
                        ...
                        onClick={() => {
                          window.open(
                            `https://twitter.com/intent/tweet?text=Join%20me%20in%20the%20${encodeURIComponent(
                              selectedGroup.media.title
                            )}%20group%20on%20Medix!&url=${encodeURIComponent(
