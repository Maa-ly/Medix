...
// Storage keys
const STORAGE_KEYS = {
  SESSION: "medix_session",
  AUTH_STATE: "medix_auth_state",
  LAST_SYNC: "medix_last_sync",
} as const;

// Session data interface
export interface SessionData {
  isConnected: boolean;
  authMethod: "walletconnect" | null;
  currentAccount: {
    address: string;
    network: string;
  } | null;
...
export const listenForExtensionSession = (
  ...
  const handler = (event: MessageEvent) => {
    ...
    if (
      ...
      message.source === "medix-extension"
    ) {
      console.log(
...
export const setupContentScriptBridge = (): void => {
  ...
  const port = chrome.runtime.connect({ name: "medix-bridge" });
