/**
 * Medix Media Tracker Content Script
...
function countMangaPages(): { current: number; total: number } {
  ...
  if (images.length === 0) {
    console.log("[Medix] No manga images found");
    return { current: 0, total: 0 };
  }

  console.log("[Medix] Found", images.length, "manga pages");
...
function startScrollTracking() {
  ...
  console.log("[Medix] Starting scroll/reading tracking");
  ...
  const updateScrollProgress = () => {
    ...
    if (currentSession) {
      ...
      if ((isScrollComplete || isPageComplete) && !currentSession.completed) {
        console.log("[Medix] Reading completed!", {
          maxScrollProgress,
          pageInfo,
        });
...
function startActivityTracking() {
  ...
  console.log("[Medix] Starting activity-based tracking");
    ...
    if (currentSession && isUserActive) {
      ...
      if (
        ...
        if (
          ...
          console.log("[Medix] Detected completion indicators");
          currentSession.completed = true;
          notifyCompletion(currentSession);
        }
      }
...
function sendTrackingUpdate() {
  ...
  try {
    ...
    console.log("[Medix] Could not send tracking update:", error);
  }
}
...
if (typeof chrome !== "undefined" && chrome.storage) {
  chrome.storage.local.get(["customSites"], (result) => {
    ...
    console.log("[Medix] Loaded custom sites:", customSites);
  });
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === "local" && changes.customSites) {
      ...
      console.log("[Medix] Updated custom sites:", customSites);
    }
  });
}
...
function parseFilmboomResponse(
  ...
  try {
    ...
    console.error("[Medix] Error parsing Filmboom response:", error);
    return null;
  }
}
...
function parseMangaDexResponse(
  ...
  try {
    ...
    console.error("[Medix] Error parsing MangaDex response:", error);
    return null;
  }
}
...
function parse9AnimeResponse(
  ...
  try {
    ...
    console.error("[Medix] Error parsing 9anime response:", error);
    return null;
  }
}
  ...
  window.fetch = async function (...args) {
    ...
    try {
      ...
      if (url.includes("wefeed-h5-bff/web/subject/detail")) {
        ...
        if (metadata) {
          console.log("[Medix] Captured Filmboom metadata:", metadata);
      ...
      if (url.includes("api.mangadex.org/chapter")) {
        ...
        if (metadata) {
          console.log("[Medix] Captured MangaDex metadata:", metadata);
          updateSessionWithMetadata(metadata);
        }
      }
      ...
      if (url.includes("/ajax/episode/") || url.includes("servers?episodeid")) {
        ...
        if (metadata) {
          console.log("[Medix] Captured 9anime metadata:", metadata);
          updateSessionWithMetadata(metadata);
        }
      }
    } catch {
...
function detectPlatform(): {
  ...
  for (const site of customSites) {
    ...
    if (hostname.includes(siteDomain) || siteDomain.includes(hostname)) {
      console.log("[Medix] Matched custom site:", site.name, siteDomain);
      return {
...
function extractMediaInfo(): MediaInfo | null {
  ...
  try {
    ...
    if (contentType === "manga" || contentType === "book") {
      console.log("[Medix] Extracting manga/book info...");
      ...
      if (
        ...
        console.log("[Medix] Webtoon title extracted:", {
          seriesTitle,
          episodeTitle,
          combinedTitle: title,
        });
      ...
      console.log("[Medix] Manga info extracted:", {
        title,
        chapter,
        episode,
        progress,
    ...
    // ========== VIDEO CONTENT EXTRACTION ==========
      ...
      if (video) {
        ...
        if (video.duration && !isNaN(video.duration) && video.duration > 0) {
          ...
          console.log("[Medix] Video element found:", {
            duration,
          ...
          console.log(
            "[Medix] Video element found but duration not ready yet"
          );
        }
        ...
        if (ytPlayer) {
          console.log(
            "[Medix] YouTube player container found, video may be loading"
          );
        }
      }
      ...
      if (name === "youtube") {
        ...
        for (const selector of titleSelectors) {
          ...
          if (el?.textContent?.trim()) {
            title = el.textContent.trim();
            console.log(
              "[Medix] YouTube title found with selector:",
              selector,
              title
            );
            break;
        ...
        if (title === "Unknown Title" && document.title) {
          ...
          if (docTitle && docTitle.length > 2 && docTitle !== "YouTube") {
            title = docTitle;
            console.log("[Medix] YouTube title from document.title:", title);
          }
        }
      } else {
        ...
        if (!video || !video.duration) {
          ...
          if (iframe) {
            console.log(
              "[Medix] Video iframe detected, using activity tracking"
            );
    ...
    if (title === "Unknown Title" && !isMangaOrBook && !isYouTube) {
      console.log(
        "[Medix] Could not extract title for video content, skipping"
      );
      return null;
    }
    ...
    if (isYouTube && title !== "Unknown Title") {
      console.log(
        "[Medix] YouTube: starting tracking with title, will update duration later"
      );
    }
    ...
    console.log("[Medix] Extracted media info:", {
      platform: name,
      type: contentType,
      title,
      progress,
    ...
    console.error("[Medix] Failed to extract media info:", error);
    return null;
  }
}
...
function startTracking() {
  ...
  if (!mediaInfo) {
    console.log("[Medix] Could not extract media info, retrying...");
...
function initSession(mediaInfo: MediaInfo) {
  console.log("[Medix] Starting media tracking:", mediaInfo);
  ...
  console.log("[Medix] Created session:", {
    id: sessionId,
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          console.error(
            "[Medix] Error sending TRACKING_START:",
            chrome.runtime.lastError
          );
        } else {
          console.log("[Medix] TRACKING_START sent successfully:", response);
        }
      }
    );
  } catch (error) {
    console.error("[Medix] Exception sending TRACKING_START:", error);
  }
  ...
  try {
    chrome.runtime.sendMessage(
      ...
      (_response) => {
        if (chrome.runtime.lastError) {
          console.error(
            "[Medix] Error sending TRACKING_UPDATE:",
            chrome.runtime.lastError
          );
        }
      }
    ...
    console.error("[Medix] Exception sending TRACKING_UPDATE:", error);
  }
  ...
  try {
    chrome.storage.local.get(["activeTracking"], (result) => {
      ...
      chrome.storage.local.set({ activeTracking: updatedTracking }, () => {
        if (chrome.runtime.lastError) {
          console.error(
            "[Medix] Error saving tracking to storage:",
            chrome.runtime.lastError
          );
        }
      });
    ...
    console.error("[Medix] Failed to write tracking to storage:", err);
  }
}
...
function stopTracking() {
  ...
  if (currentSession) {
    ...
    try {
      ...
      console.error("[Medix] Failed to remove tracking from storage:", err);
    }
...
function notifyCompletion(session: TrackingSession) {
  console.log("[Medix] Media completed!", session);
...
function showTrackSeriesButton() {
  const existing = document.getElementById("medix-track-series");
  if (existing) return;
...
function init() {
  ...
  if (!platform) {
    console.log(
      "[Medix] Not a supported media platform, but bridge is active for session sync"
    );
    return;
  }

  console.log(
    "[Medix] Content script initialized on:",
    platform.name,
    "type:",
  ...
  if (isMangaOrBook) {
    console.log(
      "[Medix] Manga/book content detected, starting tracking immediately"
    );
    ...
    const tryStartTracking = () => {
      if (!currentSession) {
        console.log("[Medix] Trying to start video tracking...");
        startTracking();
    ...
    setTimeout(() => {
      if (!currentSession) {
        console.log("[Medix] Attempting initial video tracking...");
        tryStartTracking();
      }
    }, 3000);
  }
...
function setupExtensionBridge() {
  // Message types
  const MESSAGE_TYPES = {
    SESSION_DATA: "MEDIX_SESSION_DATA",
    REQUEST_SESSION: "MEDIX_REQUEST_SESSION",
    SESSION_RECEIVED: "MEDIX_SESSION_RECEIVED",
    EXTENSION_READY: "MEDIX_EXTENSION_READY",
  };

  // Storage key
  const STORAGE_KEY = "medix_session";

  // Connect to extension background
  let port: chrome.runtime.Port | null = null;
  try {
    port = chrome.runtime.connect({ name: "medix-bridge" });
    console.log("[Medix Bridge] Connected to extension background");
    ...
    console.log("[Medix Bridge] Could not connect to background");
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    if (message && message.source === "medix-web") {
      console.log(
        "[Medix Bridge] Received from web app:",
        message.type,
        message
      );

      // Handle SESSION_DATA or SESSION_SYNC for WalletConnect
      if (
        ...
        console.log(
          "[Medix Bridge] Ignoring non-WalletConnect session from web page:",
          message.authMethod
        );
      }
  ...
  console.log("[Medix Bridge] Extension bridge initialized");
}

// Start initialization
console.log("[Medix] Initializing content script...");
...
document.addEventListener(
  ...
  (e) => {
    ...
    if (el && !currentSession) {
      console.log("[Medix] Video play event detected, starting tracking");
      startTracking();
    }
  },
  true
...
document.addEventListener(
  ...
  (e) => {
    ...
    if (el && el.currentTime > 0 && !currentSession) {
      console.log("[Medix] Video timeupdate detected, starting tracking");
      startTracking();
    }
  },
  true
...
console.log("[Medix] Calling init()...");
init();
console.log("[Medix] init() completed");
