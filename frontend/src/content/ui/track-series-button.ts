...
export function showTrackSeriesButton(
  ...
  // Click handler
  btn.onclick = () => {
    console.log("[Medix] Track Series button clicked");
    ...
    console.log("[Medix] Adding series entry:", entry);
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        ...
        if (chrome.runtime.lastError) {
          console.log(
            "[Medix] Background message failed, saving directly to storage"
          );
          saveDirectlyToStorage(entry);
        } else {
          console.log("[Medix] Series saved via background");
          showSuccessToast("✓ Series added to tracking");
        }
      }
    );
  } catch (error) {
    console.error("[Medix] sendMessage error:", error);
    saveDirectlyToStorage(entry);
  }
}
      ...
      chrome.storage.local.set(
        ...
        () => {
          if (chrome.runtime.lastError) {
            ...
            console.log("[Medix] Series saved to storage");
            showSuccessToast("✓ Series added to tracking");
          }
        }
      );
...
export function removeTrackSeriesButton(): void {
  const btn = document.getElementById("medix-track-series");
  if (btn) btn.remove();
}
