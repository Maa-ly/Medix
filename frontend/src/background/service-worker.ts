// Medix Chrome Extension - Background Service Worker
...
chrome.runtime.onInstalled.addListener((details) => {
  console.log("[Medix] Extension installed:", details.reason);
...
function setupContextMenus() {
  chrome.contextMenus.create({
    id: "medix-search",
    title: "Search on Medix",
    contexts: ["selection"],
  });

  chrome.contextMenus.create({
    id: "medix-track",
    title: "Track this media with Medix",
    contexts: ["page"],
  });
}
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === "medix-search" && info.selectionText) {
  ...
  if (info.menuItemId === "medix-track" && tab?.id) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  console.log("[Medix] Message received:", message.type);
  ...
  switch (message.type) {
    ...
    case "CHECK_WEPIN_SESSION":
      ...
      chrome.storage.local.get(["medix_wepin_session"], (result) => {
        if (result.medix_wepin_session) {
          console.log("[Medix] Wepin session found in localStorage");
          sendResponse({
            success: true,
            session: result.medix_wepin_session,
          });
        } else {
    ...
    case "TRACKING_START":
      if (sender.tab?.id) {
        ...
        console.log("[Medix] TRACKING_START received:", {
          tabId: sender.tab.id,
          data: trackingData,
        });
        ...
        console.error("[Medix] TRACKING_START: No tab ID in sender");
      }
      sendResponse({ success: true });
      return true;
    ...
    case "TRACKING_UPDATE":
      if (sender.tab?.id) {
        ...
        console.log("[Medix] TRACKING_UPDATE received:", {
          tabId: sender.tab.id,
          hasSession: !!session,
          data: updateData,
        });
        if (session) {
          ...
          console.log("[Medix] Tracking update:", {
          ...
          console.log(
            "[Medix] Created new session from update:",
    ...
    case "TRACKING_END":
      if (sender.tab?.id) {
        const session = activeSessions.get(sender.tab.id);
        if (session) {
          console.log("[Medix] Tracking ended:", {
            title: session.mediaInfo.title,
    ...
    case "MEDIA_COMPLETED":
      console.log("[Medix] Media completed!", message.data);
    ...
    case "OPEN_EXTENSION_POPUP":
      // Open extension popup to a specific route
      {
        ...
        console.log("[Medix] Opening extension popup to route:", route);
        chrome.action.setPopup({ popup: `index.html#${route}` });
        try {
          chrome.action
            .openPopup()
            .then(() => {
              console.log("[Medix] Popup opened successfully");
              sendResponse({ success: true, message: "Popup opened" });
            })
            .catch((err) => {
              console.log("[Medix] Could not open popup:", err);
              sendResponse({
                success: false,
          ...
          console.log("[Medix] openPopup error:", e);
          sendResponse({
            success: false,
    ...
    case "PENDING_MINT":
      ...
      chrome.storage.local.set({ pendingMint: message.data }, () => {
        console.log("[Medix] Pending mint stored:", message.data);
        chrome.action.setPopup({ popup: `index.html#/mint` });
        // Try to open popup, but it may fail if not triggered by user action
        try {
          chrome.action.openPopup().catch(() => {
            console.log("[Medix] Could not open popup programmatically");
          });
        } catch (e) {
          console.log("[Medix] openPopup not available:", e);
        }
      ...
      chrome.storage.local.get(["seriesBookmarks"], async (result) => {
        ...
        for (const b of list) {
          ...
          try {
            ...
            console.warn("[Medix] series check failed", e);
          }
...
function updateActiveTrackingStorage() {
  ...
  chrome.storage.local.set({ activeTracking: trackingData }, () => {
    console.log(
      "[Medix] Updated active tracking storage:",
      trackingData.length,
      "sessions",
      trackingData
    );
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo) => {
  if (changeInfo.status === "complete") {
    ...
    console.log("[Medix] Tab updated:", tabId);
  }
});
...
chrome.tabs.onRemoved.addListener((tabId) => {
  if (activeSessions.has(tabId)) {
    console.log("[Medix] Tab closed, ending session:", tabId);
  ...
  if (origins.length && scriptPaths.length) {
    ...
    if (tab?.id) {
      try {
        ...
        console.warn("[Medix] executeScript failed", e);
      }
    }
  }
});
...
chrome.permissions.onRemoved.addListener(() => {
  chrome.scripting.unregisterContentScripts({ ids: ["medix-dynamic"] });
});
...
// Session storage key
const SESSION_STORAGE_KEY = "medix_session";
...
chrome.runtime.onConnect.addListener((port) => {
  if (port.name !== "medix-bridge") return;

  console.log("[Medix Bridge] Content script connected");
  ...
  port.onMessage.addListener((message) => {
    if (message.type === BRIDGE_MESSAGE_TYPES.SESSION_DATA) {
      ...
      console.log("[Medix Bridge] Session saved from content script");
    }
  });
...
chrome.runtime.onMessageExternal.addListener(
  (request, sender, sendResponse) => {
    console.log("[Medix Bridge] External message from:", sender.url);

    // Handle SESSION_SYNC from web app (Wepin auth redirect flow)
    if (request.type === "SESSION_SYNC" && request.source === "medix-web") {
      console.log("[Medix Bridge] Received session sync from web app");
      chrome.storage.local.set({
...
console.log("[Medix Bridge] Background bridge initialized");
